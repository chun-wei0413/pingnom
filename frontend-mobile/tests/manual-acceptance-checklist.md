...
**測試步驟**:
1. [ ] **導航驗證**: 確認主選項卡新增「帳單」Tab
2. [ ] **帳單列表頁面**: 點擊「帳單」Tab，驗證帳單列表頁面正常載入
3. [ ] **建立帳單流程**:
   - [ ] 點擊「+ 新增帳單」按鈕
   - [ ] 填寫標題：「測試聚餐帳單」
   - [ ] 填寫描述：「UI 測試帳單分攤功能」
...
4. [ ] **帳單詳情頁面功能**:
   - [ ] 驗證顯示帳單標題、描述、創建者
   - [ ] 驗證顯示「草稿」狀態
   - [ ] 驗證顯示統計數據（總金額：$0，參與者：1，項目：0）

5. [ ] **新增參與者功能**:
   - [ ] 點擊「+ 新增參與者」按鈕
   - [ ] 新增 Alice Wang 為參與者 (需要實際 userID)
   - [ ] 驗證參與者列表顯示 Alice
...
**預期結果**: ✅ 帳單分攤功能完全正常，所有流程順暢執行

---

### ✅ 場景 7: Alice 帳號帳單功能測試
**目標**: 驗證 Alice 的帳單參與者視角功能

**測試步驟**:
1. [ ] 開啟新分頁，登入 Alice Wang 帳號
2. [ ] 點擊「帳單」Tab
3. [ ] 驗證帳單列表顯示剛才的測試帳單
4. [ ] 點擊進入帳單詳情
5. [ ] 驗證 Alice 看到自己的分攤金額和付款狀態
6. [ ] 點擊「標記已付」按鈕
7. [ ] 驗證自己的付款狀態更新
8. [ ] 驗證帳單狀態變更為「已完成」（如果所有人都付清）

**預期結果**: ✅ 參與者視角功能正常，付款流程完整
...
**帳單分攤系統 (新增)**:
- [ ] ✅ POST `/api/v1/bills/` → 前端建立帳單功能
- [ ] ✅ GET `/api/v1/bills/` → 前端帳單列表功能
- [ ] ✅ GET `/api/v1/bills/:id` → 前端帳單詳情功能
- [ ] ✅ POST `/api/v1/bills/:id/items` → 前端新增項目功能
- [ ] ✅ POST `/api/v1/bills/:id/participants` → 前端新增參與者功能
- [ ] ✅ PUT `/api/v1/bills/:id/payments` → 前端標記付款功能
